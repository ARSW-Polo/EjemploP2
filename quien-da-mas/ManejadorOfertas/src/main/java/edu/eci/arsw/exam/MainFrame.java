...
public class MainFrame extends javax.swing.JFrame {
    ...
    private JTextArea acceptedOffersArea;
    ...
    public MainFrame() {
        initComponents();
        acceptedOffersArea = new JTextArea();
        acceptedOffersArea.setEditable(false);
        jScrollPane2.setViewportView(acceptedOffersArea);
    }

    public void showAcceptedOffer(String productCode, String buyerCode, int monto) {
        String line = "Producto " + productCode + " vendido a " + buyerCode + " por " + monto + "\n";
        SwingUtilities.invokeLater(new Runnable() {
            @Override
            public void run() {
                acceptedOffersArea.append(line);
            }
        });
    }
    ...
    private void botonEnvioActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botonEnvioActionPerformed
        try {
            String productCode = this.codigoTareaTF.getText() == null ? "" : this.codigoTareaTF.getText().trim();
            String description = this.descripcionTareaTF.getText() == null ? "" : this.descripcionTareaTF.getText().trim();
            String startPriceText = this.startPriceField.getText() == null ? "" : this.startPriceField.getText().trim();
            ...
            if (startPriceText.isEmpty()) {
                JOptionPane.showMessageDialog(this, "Debe ingresar el precio inicial.");
                return;
            }

            int startPrice;
            try {
                startPrice = Integer.parseInt(startPriceText);
            } catch (NumberFormatException ex) {
                JOptionPane.showMessageDialog(this, "El precio inicial debe ser un número entero.");
                return;
            }
            if (startPrice < 0) {
                JOptionPane.showMessageDialog(this, "El precio inicial no puede ser negativo.");
                return;
            }
